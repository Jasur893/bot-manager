...
function custom_files($file_path)
{
    return new CURLFile($file_path); // local fayllarni ni yuborish, o'zimizni serverdan telegramga yuborish
}
...
function get_fallows($params = [])
{
    ...
    if ($params['test_btn']) {
        array_push($list_channels, [
           [
               'text_btn' => "Obuna bo'ldim ✅",
               'callback_btn' => "followed"
           ]
        ]);
    } elseif ($params['share_btn']){
        array_push($list_channels, [
            [
               'text' => $share_btn['share_btn'],
               'url' => 'https://t.me/share/url?url='.$share_btn['share_btn'].'&text='.$share_btn['share_text']
            ]
        ]);
    }
