...
$channnel_post = $update->channel_post;
$text = html($message->text);
$bot_command = $message->entities->type;
...
// guruh yoki kanalda yengi foydalanuvchi qo'shildi yoki foydalanuvchi chiqib ketdi degan tizim  xabarni o'chirib yuboradi
...
// guruhlar, mening guruhim
if ($chat_type == 'supergroup' && $chat_id == $my_group) {
    if(isset($new_members) && count($new_members) > 0 && $new_members[0]->id != $from_id){
        invites(['from_id' => $from_id, 'new_members' => count($new_members), 'name' => $full_name]);
    } elseif (mb_stripos($text, "/top@") !== false) {
        ...
        delete(['chat_id' => $chat_id, 'mess_id' => $message_id]);
    } elseif (mb_stripos($text, "/money@") !== false) {
        ...
        if ($res) {
            ...
            $reply .= "\n <pre>⚠️ Ushbu ma'lumotlar O'ZRES markaziy banki tomonidan taqdim etilgan, yangilanish vaqti bir kun !</pre>";
        } else {
            $reply .= "⚠️ Xatolik, Ushbu ma'lumotlar O'ZRES markaziy banki tomonidan taqdim etilgan, agar xatolik bilan javob olsangiz keyinroq urinib ko'ring !!!";
        }
        bot('sendMessage', [
            'chat_id' => $chat_id,
            'text' => $reply,
        ...
        delete(['chat_id' => $chat_id, 'mess_id' => $message_id]);
    } elseif ($from_id == $admin && isset($message->reply_to_message) && (mb_stripos($text, "/ban@") !== false)) {
        bot('banChatMember', [
            'chat_id' => $chat_id,
            'user_id' => $message->reply_to_message->from->id,
            'revoke_messages' => true
        ]);
    } elseif ($from_id == $admin && isset($message->reply_to_message) && (mb_stripos($text, "/mute") !== false)) {
        $params = explode("+", $text);
        $until_hours = (count($params) == 2) ? $params[1] : 1;
        bot('restrictChatMember', [ // promoteChatMember teskarisi
            'chat_id' => $chat_id,
            'user_id' => $message->reply_to_message->from->id,
        ...
        delete(['chat_id' => $chat_id, 'mess_id' => $message_id]);
    } elseif ($from_id == $admin && (mb_stripos($text, "/stop@") !== false)) {
        ...
        delete(['chat_id' => $chat_id, 'mess_id' => $message_id]);
    } elseif ($from_id == $admin && (mb_stripos($text, "/start@") !== false)) {
        bot('setChatPermissions', [
            'chat_id' => $chat_id,
            'permissions' => json_encode([
        ...
        delete(['chat_id' => $chat_id, 'mess_id' => $message_id]);
    } elseif (mb_stripos($text, "/game@") !== false) {
        // $dices = ["🎲", "🎯", "🎳", "🏀", "⚽", "🎰"];
        bot('sendDice', [
            'chat_id' => $chat_id,
...
if ($chat_type == 'private' && $chat_id != $admin) {
    if (user_is_followed($from_id)) {
        if ($text == "/start") {
            ...
            if ($res = game(['game' => $game['result']])) {
                bot('sendMessage', [
                    'chat_id' => $chat_id,
                    'text' => "Botning natijasi: <b>".$res['value']." ball</b> 😎 👈 .\n 🤝 Bot bilan o'yinda g'olibni aniqlash uchun, siz ham ushbu o'yin ustiga bosib botga yuboring !!!<pre>⚠ Bot har safar tasodifiy o'yinni yuboradi va natijalar omadga bog'liq bo'ladi.</pre>  ",
                    'parse_mode' => "HTML",
                ]);
            }
            ...
            if ($res = game(['mess' => $message])) {
                ...
                $reply .= "\nYana o'ynash uchun 👉 /game buyrug'ini bosing !";
            } else {
            ...
            bot('sendMessage', [
                'chat_id' => $chat_id,
                'text' => "Guruhga takliflar bo'yicha sizning ballaringiz: <b>" . $res . "</b>🎯. <pre>Ballarni ko'paytirish uchun bizning guruhimizga do'stlaringizni taklif qiling</pre>",
                'parse_mode' => 'HTML',
                'reply_markup' => json_encode([
        ...
        bot('sendMessage', [
            'chat_id' => $chat_id,
            'text' => "⚠️ Xatolik, tizimdan foydalanish uchun bizning kanallarga a'zo bo'lishingiz shart !",
            'parse_mode' => 'HTML',
 ...
 //mycloud+3 File haqida, guruhlarga va kannallarga tegishli, chatga bog'lanmagan botimiz kanal va guruhda admin roli bo'lsa ishlaydi
